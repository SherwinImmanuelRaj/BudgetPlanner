...
// Budget Planning App - Enhanced with Direct File Modification
...
class BudgetApp {
    constructor() {
        ...
        // File paths in userdata folder
        this.dataFilePath = 'userdata/budget-data.json';
        this.templatesFilePath = 'userdata/fixed-expense-templates.json';
        this.debtTemplatesFilePath = 'userdata/debt-templates.json';
        this.themeFilePath = 'userdata/theme-setting.json';
    ...
    async init() {
        try {
            ...
            // Load data with direct file system
    ...
    // Direct file modification methods using localStorage as fallback
    async saveToFile(filePath, data) {
        try {
            const dataStr = JSON.stringify(data, null, 2);
            
            // Store in localStorage as primary method (since we can't directly write to files in browser)
            const key = filePath.replace('userdata/', '').replace('.json', '');
            localStorage.setItem(`budget-${key}`, dataStr);
            
            console.log(`Data saved to ${filePath} via localStorage`);
        } catch (error) {
    ...
    async loadFromFile(filePath, defaultData = null) {
        try {
            // Try to load from localStorage first
            const key = filePath.replace('userdata/', '').replace('.json', '');
            const stored = localStorage.getItem(`budget-${key}`);
            
            if (stored) {
                return JSON.parse(stored);
            }

            // Try to fetch from the actual file
            try {
                ...
                console.log(`Could not fetch ${filePath}, using localStorage or default`);
            }

            // If no stored data and fetch failed, return default
            return defaultData;
        } catch (error) {
    ...
    async setupTheme() {
        try {
            const savedTheme = await this.getStoredTheme() || 'light';
    ...
    async loadData() {
        try {
            const data = await this.loadFromFile(this.dataFilePath);
            if (data) {
                return data;
            }
    ...
    async loadFixedExpenseTemplates() {
        try {
            return await this.loadFromFile(this.templatesFilePath, []);
        } catch (error) {
    ...
    async loadDebtTemplates() {
        try {
            return await this.loadFromFile(this.debtTemplatesFilePath, []);
        } catch (error) {
    ...
    async saveData() {
        try {
            await this.saveToFile(this.dataFilePath, this.data);
    ...
    async saveFixedExpenseTemplates() {
        try {
            await this.saveToFile(this.templatesFilePath, this.fixedExpenseTemplates);
        } catch (error) {
    ...
    async saveDebtTemplates() {
        try {
            await this.saveToFile(this.debtTemplatesFilePath, this.debtTemplates);
        } catch (error) {
    ...
    async getStoredTheme() {
        try {
            const themeData = await this.loadFromFile(this.themeFilePath, { theme: 'light' });
    ...
    async saveTheme(theme) {
        try {
            await this.saveToFile(this.themeFilePath, { theme: theme });
        } catch (error) {
